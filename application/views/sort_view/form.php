				...
				<div class="row">
					<form method="POST" id="postdata" action="<?=base_url($controller . '/aksi/' . $action . '/' . $intid)?>">
						<div class="col-md-6">
							...
							<div class="form-group">
								<label>Sort OEE</label>
								<input type="number" name="intsort" placeholder="Sort OEE" class="form-control" id="intsort" required value="<?=$intsort?>" />
							</div>
							<div class="form-group">
								<label>Sort All</label>
								<input type="number" name="intsortall" placeholder="Sort All" class="form-control" id="intsortall" required value="<?=$intsortall?>" />
							</div>
...
<script type="text/javascript">
	function simpanData(action) {
		...
		var intsort      = $('#intsort').val();
		var intsortall   = $('#intsortall').val();
