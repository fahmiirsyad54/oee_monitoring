...
<div class="modal fade" id="loadingdowntime" role="dialog">
...
<script>
    $(function (){
        ...
        $('#loadingdowntime').modal({ backdrop: 'static' },'show');
        $.ajax({
            url: base_url + '/downtime__ajax/<?=$intgedung?>/<?=$from?>/<?=$to?>/<?=$intshift?>',
            method: "GET"
        })
        .done(function( data ) {
          $('#loadingdowntime').modal('hide');
