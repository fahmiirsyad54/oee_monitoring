				...
				<form method="POST" id="postdata" action="<?=base_url($controller . '/aksi/' . $action . '/' . $intid)?>">
					...
					<div class="row">
						...
						<div class="col-md-3">
							<div class="form-group">
								...
								<select name="intsparepart" class="form-control select2" id="intsparepart">
									...
									?>
									<option <?=$selected?> data-nama="<?=$opt->vcnama?>" value="<?=$opt->intid?>"><?=$opt->vcnama . $vcspekpart?></option>
									<?php
										}
									?>
								</select>
						...
						<div class="col-md-1">
							<div class="form-group">
								<label>Qty</label>
								<input type="text" name="intjumlah" placeholder="Qty" class="form-control" id="intjumlah" required value="<?=$intjumlah?>" />
							</div>
						</div>
					</div>
