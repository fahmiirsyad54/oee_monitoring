					...
					<form method="POST" id="postdata" action="<?=base_url($controller . '/aksi/' . $action . '/' . $intid)?>">
						<div class="col-md-6">
							...
							<div class="form-group">
								<label>Proses Group</label>
								<select class="form-control" name="intprosesgroup" id="intprosesgroup">
									<option>-- Select Procces Group --</option>
									<?php
										foreach ($listprosesgroup as $opt) {
											$selected = ($opt->intid == $intprosesgroup) ? 'selected' : '';
									?>
