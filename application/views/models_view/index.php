							...
							?>
								<tr>
									...
									<td>
										...
										<a href="javascript:void(0)" onclick="addSME2(<?=$data->intid?>)" class="btn btn-primary btn-xs">Add / Edit SME</a>
...
<!-- Modal -->
<div id="modalSME2" class="modal fade" role="dialog">
	<div class="modal-dialog modal-lg">
		<!-- Modal content-->
		<div class="modal-content" id="datasme2">
		</div>
	</div>
</div>
<script type="text/javascript">
	...
	function addSME2(intid) {
		var base_url = '<?=base_url($controller)?>';
		$.ajax({
			url: base_url + '/addsme2/' + intid,
			method: "GET"
		})
		.done(function( data ) {
			$('#datasme2').html(data);
			$('#modalSME2').modal('show');
		})
		.fail(function( jqXHR, statusText ) {
			alert( "Request failed: " + jqXHR.status );
		});
