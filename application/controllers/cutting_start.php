...
class cutting_start extends MY_Controller { 

    function __construct(){
        parent::__construct();
        date_default_timezone_set('Asia/Jakarta');
        $this->load->model('AppModel');
        $this->load->model('Cutting_startModel');
        $this->model = $this->Cutting_startModel;
    ...
    function view($halaman=1){
        ...
        if ($intshift == 0) { 
            ...
            $date2   = date( "Y-m-d 23:59:59", strtotime( $from) );
            ...
            $jmlpage = (ceil($jmldata[0]->jmldata > 45) ? (45 / $this->limit) : ceil($jmldata[0]->jmldata) / $this->limit);
    ...
    function exportexcelnew(){
        ...
        $excel->getProperties()->setCreator('')
                     ->setLastModifiedBy('')
                     ->setTitle("Report Cutting Start " . $judul . '-' . $vcshift)
                     ->setSubject("Report Cutting Start")
                     ->setDescription("Report Cutting Start")
                     ->setKeywords("Report Cutting Start");
        ...
        foreach ($dtgedung as $gedung) {
            ...
            $excel->setActiveSheetIndex($loop)->setCellValue('B1', "Report CUtting Start " . $gedung->vcnama);
            $excel->getActiveSheet()->mergeCells('B1:I1'); // Set Merge Cell
            $excel->getActiveSheet()->getStyle('B1')->getFont()->setBold(TRUE); // Set bold
            $excel->getActiveSheet()->getStyle('B1')->getFont()->setSize(15); // Set font size 15
            $excel->getActiveSheet()->getStyle('B1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER); // Set text center

            $excel->setActiveSheetIndex($loop)->setCellValue('B2', "Report CUtting Start, on Date : ". date('d-m-Y',strtotime($from)) . " ( ". $vcshift . ")");
            $excel->getActiveSheet()->mergeCells('B2:I2'); // Set Merge Cell
            $excel->getActiveSheet()->getStyle('B2')->getFont()->setBold(TRUE); // Set bold
            $excel->getActiveSheet()->getStyle('B2')->getFont()->setSize(12); // Set font size 15
            $excel->getActiveSheet()->getStyle('B2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT); // Set text center

            $excel->setActiveSheetIndex($loop)->setCellValue('B3', "NO");
            $excel->setActiveSheetIndex($loop)->setCellValue('C3', "ID Machine");
            $excel->setActiveSheetIndex($loop)->setCellValue('D3', "Name Machine");
            $excel->setActiveSheetIndex($loop)->setCellValue('E3', "Cell");
            $excel->setActiveSheetIndex($loop)->setCellValue('F3', "Login Start");
            $excel->setActiveSheetIndex($loop)->setCellValue('G3', "Cutting Start");
            $excel->setActiveSheetIndex($loop)->setCellValue('H3', "Duration");
            $excel->setActiveSheetIndex($loop)->setCellValue('I3', "Duration on second");
            ...
            foreach ($dtmesin as $mesin) {
                ...
                foreach ($data as $dataset) {
                    ...
                    $durasi = abs(strtotime($dataset->dtmulai) - strtotime(date('H:i:s', strtotime($dataset->dtlogin))))  ;
                    ...
                    $excel->setActiveSheetIndex($loop)->setCellValue('B'.$numrow, ++$no);
                    $excel->setActiveSheetIndex($loop)->setCellValue('C'.$numrow, $dataset->vcmesin);
                    $excel->setActiveSheetIndex($loop)->setCellValue('D'.$numrow, $dataset->vcnamamesin);
                    $excel->setActiveSheetIndex($loop)->setCellValue('E'.$numrow, $dataset->vccell);
                    $excel->setActiveSheetIndex($loop)->setCellValue('F'.$numrow, date('H:i:s', strtotime($dataset->dtlogin)));
                    $excel->setActiveSheetIndex($loop)->setCellValue('G'.$numrow, $dataset->dtmulai);
                    $excel->setActiveSheetIndex($loop)->setCellValue('H'.$numrow, $GAP);
                    $excel->setActiveSheetIndex($loop)->setCellValue('I'.$numrow, $durasi);
        ...
        // Proses file excel
        header('Content-Type: application/vnd.ms-excel'); //mime type
        header('Content-Disposition: attachment; filename="Report Cutting Start ' .$judul. '.xls"'); // Set nama file excel nya
        header('Cache-Control: max-age=0');
