...
class Sort extends MY_Controller {
    ...
    function view($halaman=1){
        ...
        $data['dataP']      = $this->model->getdatalimit($this->table,$offset,$this->limit,$keyword,1);
        $data['dataP2']     = $this->model->getdatalimit2($this->table,$offset,$this->limit,$keyword,1);
    ...
    function edit($intid){
        ...
        $data = array(
                    ...
                    'intsort'    => $resultData[0]->intsort,
                    'intsortall' => $resultData[0]->intsortall,
                    'intupdate'  => $this->session->intid,
                    'dtupdate'   => date('Y-m-d H:i:s')
                );
    ...
    function aksi($tipe,$intid,$status=0){
        if ($tipe == 'Add') {
            ...
            if ($result) {
             ...
             $vcnama     = $this->input->post('vcnama');
             $intsort    = $this->input->post('intsort');
             $intsortall = $this->input->post('intsortall');
           
            $data    = array(
                    'vcnama'     => $vcnama,
                    'intsort'     => $intsort,
                    'intsortall' => $intsortall,
                    'intupdate'  => $this->session->intid,
                    'dtupdate'   => date('Y-m-d H:i:s')
                );
