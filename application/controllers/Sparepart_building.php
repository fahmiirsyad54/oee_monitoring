...
class Sparepart_building extends MY_Controller {

	function __construct(){
        parent::__construct();
        $this->load->model('Sparepart_buildingModel');
        $this->model = $this->Sparepart_buildingModel;
    }
    ...
    function view($halaman=1){
        ...
        if ($intsparepart != '' && $dtmonth != '') {
            if ($intsparepart == 0) {
                $jmldata    = $this->model->jumlahreportall($dtmonth,$intyear,$intgedung);
                $offset     = ($halaman - 1) * $this->limit;
                $jmlpage    = ceil($jmldata / $this->limit);
                $datareport = $this->model->getreportalllimit($dtmonth,$intyear,$intgedung,$offset,$this->limit);
            } elseif ($intsparepart > 0) {
                $jmldata    = $this->model->jumlahreportpersparepart($intsparepart,$dtmonth,$intyear,$intgedung);
                $offset     = ($halaman - 1) * $this->limit;
                $jmlpage    = ceil($jmldata / $this->limit);
                $datareport = $this->model->getreportpersparepatlimit($intsparepart,$dtmonth,$intyear,$intgedung,$offset,$this->limit);
            }
        }
