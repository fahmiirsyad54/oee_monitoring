...
class Operator extends CI_Controller {
    ...
    function getstatuspo($intmodel, $intkomponen, $vcpo) {
        $allow_po     = $this->modelapp->getappsetting('allowance_po')[0]->vcvalue;
        $dataloadplan = $this->model->cekpo($intmodel, $vcpo);
        $jumlahpasang = 0;

        if (count($dataloadplan) == 0) {
            ...
            //$dataloadplan = $this->model->cekpo($intmodel, $vcpo);
            $dataaktual   = $this->model->getaktual($intmodel, $intkomponen, $dataloadplan[0]->vcpo);
            if ($dataaktual[0]->jumlahpasang >= ($dataloadplan[0]->jumlahloadplan + round($dataloadplan[0]->jumlahloadplan * ($allow_po/100)))) {
                $status       = 1;  // target po terpenuhi
                $jumlahpasang = 0;
                $intqty       = 0;
                $poasli       = '';
            } else {
                $status       = 2; // ok
                $jumlahpasang = ($dataaktual[0]->jumlahpasang) ? $dataaktual[0]->jumlahpasang : 0 ;
                $intqty       = $dataloadplan[0]->jumlahloadplan + round($dataloadplan[0]->jumlahloadplan * ($allow_po/100));
                $poasli       = $dataloadplan[0]->vcpo;
            }
        }
